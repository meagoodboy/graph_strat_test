...
# enc_np = np.ascontiguousarray(enc_np, dtype=np.float32)
print(enc_np.shape)

fpdnmf= FPDNMF(enc_np, rank=rank)
fpdnmf.compute_factors(max_iter=max_iter, nditer=5)

W  = pd.DataFrame(fpdnmf.W, index=enc.index)
...
plot.set(title="FPDNMF")
fig = plot.get_figure()
fig.savefig(save_name+ "_fpdnmf_scatter.png")

data.to_csv(save_name+"_fpdnmf.csv")
