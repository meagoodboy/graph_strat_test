...
def cluster_KMplot(cluster_assign, clin_data_fn, delimiter='\t', lr_test=True, tmax=-1, verbose=True, **save_args):
    title = 'Survival Plot'
    if 'job_name' in save_args:
        title = save_args['job_name']+' Survival Plot'
    ...
    for clust in clusters:
        ...
        clust_pats = [x for x in clust_pats if str(x) != 'nan']
        ...
        kmf.plot(ax=ax, color=cluster_cmap[clust], ci_show=False, show_censors=True)
    ...
    if lr_test:
        ...
        plt.title(title, fontsize=24, y=1.02)
    xcor, ycor = 250, 0.7
    plt.text(xcor, ycor, "p="+repr(round(p, 4)), fontsize=24)
    # Save KM plot
    if 'outdir' in save_args:
        if 'job_name' in save_args:
            save_KMplot_path = save_args['outdir'] + str(save_args['job_name'])
        else:
            save_KMplot_path = save_args['outdir']
        plt.savefig(save_KMplot_path+'_KM_plot.png', bbox_inches='tight')

#         for tiff
        png1 = BytesIO()
        fig.savefig(png1, format='png')
        png2 = Image.open(png1)
        png2.save(save_KMplot_path+'_KM_plot.tiff')
        png1.close()
